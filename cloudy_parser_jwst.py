...
def get_emergent_lines(grid):
    ...
    for l in grid:
        ...
        if "blends..............................." in l:
            go = False
        if go:
            if ('...' not in l) and ('TOTL' not in l) and ('***' not in l) and (not cp) and (len(l)>2) and ('Ca A' not in l) and ('Ca B' not in l) and ('nFnu' not in l) and ('MIR' not in l) and ('PAH' not in l) and ('F' not in l)  and ('IR' not in l):
                lines.append(parse_line_line(l))
                wls.append(lines[-1][2])
        cp = False
    lines = [x for _, x in sorted(zip(wls, lines))]
